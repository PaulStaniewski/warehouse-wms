...
| `/api/route-round-schedules/` | Authenticated; branch scoped reads. Create/update/delete require Leader in the schedule route branch. |
| `/api/branch-dispatch-policies/` | Authenticated; branch scoped reads. Create/update/delete require Leader in the policy branch. |
| `POST /api/route-runs/{id}/override-times/` | Authenticated Leader-only command for the RouteRun branch. Closed/cancelled/dispatched runs are not mutable. |
| `GET /api/shipments/route-targets/` | Authenticated; branch scoped route-change target lookup. Returns existing eligible RouteRuns and demand-created scheduled slots for the requested branch/date scope. |
| `POST /api/shipments/{id}/change-route/` | Authenticated same-branch command. Target RouteRun or schedule slot must belong to the shipment branch and pass state/date eligibility. |
